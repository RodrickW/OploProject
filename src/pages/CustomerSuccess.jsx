...
export default function CustomerSuccess() {
  ...
  const subPages = [
    { id: 'overview', label: isEn ? 'Overview' : 'Vue d\'ensemble', icon: Users },
    { id: 'analytics', label: isEn ? 'Customer Analytics' : 'Analytique client', icon: BarChart3 },
    { id: 'feedback', label: isEn ? 'Feedback Tracker' : 'Suivi feedback', icon: MessageSquare },
    { id: 'tickets', label: isEn ? 'Support Tickets' : 'Tickets support', icon: HeadphonesIcon },
    { id: 'journey', label: isEn ? 'Customer Journey' : 'Parcours client', icon: Route },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500">{isEn ? 'Manage customer satisfaction and loyalty' : 'Gérez la satisfaction et la fidélité de vos clients'}</p>
        </div>
      </div>
          ...
          {!hasCustomerData ? (
            <div className="rounded-xl bg-white border border-gray-200 p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {isEn ? 'Connect your POS system' : 'Connectez votre système de caisse'}
              </h3>
              <p className="text-gray-500 mb-6">
                {isEn
                  ? <>Customer data comes from your integrated POS or CRM.<br/>Go to <strong>Oplo Integrations</strong> to connect your system.</>
                  : <>Les données clients proviennent de votre POS ou CRM intégré.<br/>Rendez-vous dans <strong>Intégrations Oplo</strong> pour connecter votre système.</>
                }
              </p>
              <Button 
              ...
              >
                {isEn ? 'Configure integrations' : 'Configurer les intégrations'}
              </Button>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm text-gray-500">{isEn ? 'Total customers' : 'Total clients'}</span>
                  </div>
                ...
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm text-gray-500">{isEn ? 'New (30d)' : 'Nouveaux (30j)'}</span>
                  </div>
                ...
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm text-gray-500">{isEn ? 'VIP Customers' : 'Clients VIP'}</span>
                  </div>
                  <p className="text-2xl font-bold text-gray-900">{stats.vip.toLocaleString()}</p>
                  <p className="text-xs text-gray-500 mt-2">{isEn ? 'Top 5% spenders' : 'Top 5% des dépenses'}</p>
                </div>
                
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <UserMinus className="w-5 h-5 text-red-600" />
                    <span className="text-sm text-gray-500">{isEn ? 'At risk' : 'À risque'}</span>
                  </div>
                  <p className="text-2xl font-bold text-gray-900">{stats.atRisk.toLocaleString()}</p>
                  <p className="text-xs text-gray-500 mt-2">{isEn ? 'Last visit > 60d' : 'Dernière visite > 60j'}</p>
                </div>
              </div>

              <div className="grid lg:grid-cols-2 gap-6">
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  <h3 className="text-sm font-medium text-gray-600 mb-4">{isEn ? 'Customer satisfaction' : 'Satisfaction client'}</h3>
                ...
                <div className="rounded-xl bg-white border border-gray-200 p-5">
                  ...
                  <div className="flex items-center justify-center py-4">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 mt-2">{isEn ? 'Excellent' : 'Excellent'}</p>
                    </div>
                  </div>
                  <div className="flex justify-between text-xs text-gray-500 mt-4">
                    <span>{isEn ? 'Detractors: 8%' : 'Détracteurs: 8%'}</span>
                    <span>{isEn ? 'Passives: 12%' : 'Passifs: 12%'}</span>
                    <span>{isEn ? 'Promoters: 80%' : 'Promoteurs: 80%'}</span>
                  </div>
                </div>
              </div>

              <div className="rounded-xl bg-white border border-gray-200 p-5">
                <h3 className="text-sm font-medium text-gray-600 mb-4">{isEn ? 'Latest reviews' : 'Derniers avis'}</h3>
                <div className="space-y-3">
                  {feedbacks.length === 0 ? (
                    <p className="text-gray-500 text-center py-8">{isEn ? 'No recent feedback' : 'Aucun feedback récent'}</p>
                  ) : (
                    feedbacks.slice(0, 5).map(feedback => (
                      <div key={feedback.id} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-100">
                        ...
                        <div className="flex-1">
                          <p className="text-sm text-gray-700">{feedback.comment || (isEn ? 'No comment' : 'Pas de commentaire')}</p>
                          <div className="flex items-center gap-2 mt-2">
                            <Badge variant="outline" className="border-gray-200 text-gray-600 text-xs">
                              {feedback.category || (isEn ? 'General' : 'Général')}
                            </Badge>
