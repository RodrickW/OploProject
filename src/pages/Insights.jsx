          ...
          <div className="flex-1 min-w-0">
            ...
            {insight.impact_value && (
              <div className="flex items-center gap-2 mb-4 text-sm">
                ...
                <span className="text-gray-600">Impact estimé:</span>
                <span className="font-semibold text-emerald-600">
                  €{insight.impact_value.toLocaleString()}
                </span>
              </div>
            )}
            ...
            {insight.recommended_action && (
              <div className="bg-gray-50 rounded-lg p-3 mb-4">
                <p className="text-xs text-gray-500 mb-1">Action recommandée</p>
            ...
            <div className="flex items-center gap-2">
              {insight.status === 'new' && (
                ...
                >
                  <Clock className="w-4 h-4 mr-1" /> Prendre en charge
                </Button>
              )}
              {insight.status === 'in_progress' && (
                <Button 
                ...
                >
                  <CheckCircle className="w-4 h-4 mr-1" /> Marquer résolu
                </Button>
              )}
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Analyse en temps réel de vos restaurants</p>
        </div>
          ...
          <div className="flex items-center gap-3">
            {criticalCount > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-100 border border-red-300">
                ...
                <span className="text-sm font-medium text-red-700">{criticalCount} critique(s)</span>
              </div>
            )}
            {highCount > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-100 border border-amber-300">
                <TrendingUp className="w-4 h-4 text-amber-600" />
                <span className="text-sm font-medium text-amber-700">{highCount} important(s)</span>
              </div>
            )}
          </div>
        )}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-gray-100 border border-gray-200">
          <TabsTrigger value="all">Tous ({insights.length})</TabsTrigger>
          <TabsTrigger value="critical" className="text-red-600">
            Critiques ({insights.filter(i => i.priority === 'critical').length})
          </TabsTrigger>
          <TabsTrigger value="high" className="text-amber-600">
            Importants ({insights.filter(i => i.priority === 'high').length})
          </TabsTrigger>
          <TabsTrigger value="medium">
            Moyens ({insights.filter(i => i.priority === 'medium').length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value={activeTab} className="mt-6">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Aucun insight</h3>
              <p className="text-gray-500">Les insights apparaîtront automatiquement<br/>lorsque nous détecterons des opportunités</p>
            </div>
          ) : (
            <div className="grid gap-4">
