...
import StepRestaurants from '../components/onboarding/StepRestaurants';
import StepOperations from '../components/onboarding/StepOperations';
import StepTechStack from '../components/onboarding/StepTechStack';
import StepObjectives from '../components/onboarding/StepObjectives';
import StepCOMPAST from '../components/onboarding/StepCOMPAST';
...
export default function Onboarding() {
  ...
  useEffect(() => {
    base44.auth.me().then(user => {
      ...
      base44.entities.OnboardingProfile.filter({ created_by: user.email, status: 'completed' }, '-created_date', 1).then(profiles => {
        if (profiles.length > 0) window.location.href = createPageUrl('Dashboard');
      });
    }).catch(() => {});
  }, []);
  ...
  const handleComplete = async () => {
    ...
    setDone(true);
    setTimeout(() => { window.location.href = createPageUrl('Dashboard'); }, 2500);
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col" style={{ '--foreground': '0 0% 8%', '--background': '0 0% 98%', '--input': '0 0% 95%', '--muted-foreground': '0 0% 45%', '--border': '0 0% 85%', color: 'hsl(0 0% 8%)' }}>
      {/* Header */}
